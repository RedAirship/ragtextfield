...
//  
//
//  Created by Huy Nguyen on 14/06/2022.
//
...
open class RAGTextView: UITextView {
    ...
    open var leftViewMode: UITextField.ViewMode = .always {
        didSet {
            setNeedsUpdateHorizontalPlaceholderConstraints()
        }
    }
    ...
    open var rightViewMode: UITextField.ViewMode = .always {
        didSet {
            setNeedsUpdateHorizontalPlaceholderConstraints()
        }
    }
...
//        }
    ...
    open func leftViewRect(forBounds bounds: CGRect) -> CGRect {
        ...
        let superValue = leftViewRect(forBounds: bounds)
    ...
    open func rightViewRect(forBounds bounds: CGRect) -> CGRect {
        ...
        let superValue = rightViewRect(forBounds: bounds)
        let size = superValue.size
...
//            inset = bounds.width - clearButtonRect(forBounds: bounds).minX + Constants.overlaySpaceToText
    ...
    open func clearButtonRect(forBounds bounds: CGRect) -> CGRect {
        ...
        let superValue = clearButtonRect(forBounds: bounds)
        let size = superValue.size
        let y = computeTopInsetToText() + 0.5 * (measureTextHeight() - size.height)
